    ...
    constructor(props) {
        ...
        for (var i = 0; i < this.state.result.length; i++) {
            if (this.state.listsNum == this.state.result[i].resultsCode) {
                cateArray.push(this.state.result[i].cates)
            }
        }
        //Matching the cate Code with this.state.cates, Push item Code into exArray
        var exArray = [];
        this.state.itemList.map((data) => {
            for (var i = 0; i < cateArray[0].length; i++) {
                for (var j = 0; j < cateArray[0][i].item.length; j++) {
                    if (data.code === cateArray[0][i].cate && data.itemCode == cateArray[0][i].item[j]) {
                        exArray.push(data)
                    }
                }
            }
        ...
        const mapToExplanation = data => {
            if (this.state.dataSource[0] != undefined) {
                return data.map((data, i) => {
                    if (i != Math.max(this.state.dataSource.length)) {
                        return (
